...
namespace Fusion {
  ...
  public class FusionBootstrapDebugGUI : Behaviour {
    ...
    protected virtual void OnGUI() {
      ...
      GUI.skin = FusionScalableIMGUI.GetScaledSkin(
        BaseSkin,
        out var height,
        out var width,
        out var padding,
        out var margin,
        out var leftBoxMargin
      );
      ...
      GUILayout.BeginArea(new Rect(leftBoxMargin, margin, width, Screen.height));
      {
        ...
        GUILayout.BeginVertical(GUI.skin.window);
        {
          if (currentstage == FusionBootstrap.Stage.Disconnected) {
            ...
            if (GUILayout.Button(
                  EnableHotkeys ? "Start Single Player (I)" : "Start Single Player",
                  GUILayout.Height(height)
                )) nds.StartSinglePlayer();

            if (GUILayout.Button(
                  EnableHotkeys ? "Start Shared Client (P)" : "Start Shared Client",
                  GUILayout.Height(height)
                )) {
              if (_isMultiplePeerMode)
                StartMultipleSharedClients(nds);
            ...
            if (_isMultiplePeerMode) {
              GUILayout.BeginHorizontal( /*GUI.skin.button*/);
              {
                ...
                var newcount = GUILayout.TextField(
                  _clientCount,
                  10,
                  GUILayout.Width(width * .25f),
                  GUILayout.Height(height)
                );
