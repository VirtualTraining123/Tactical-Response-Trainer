...
namespace Fusion.Statistics {
  ...
  public class FusionStatsCanvas : MonoBehaviour, IDragHandler, IEndDragHandler, IBeginDragHandler {
    ...
    [Space] [Header("Panel References")] [SerializeField]
    ...
    [Space] [Header("Misc")] [SerializeField]
    private Button _hideButton;

    [SerializeField] private Button _closeButton;

    [Space] [Header("World Anchor Panel Settings")] [SerializeField]
    ...
    internal void SetupStatsCanvas(FusionStatistics fusionStatistics, CanvasAnchor canvasAnchor,
      UnityAction closeButtonAction) {
      ...
      _canvasPanel.anchoredPosition +=
        Vector2.down * (FusionStatisticsHelper.DEFAULT_HEADER_HEIGHT * maxOffsetMultiplier);
    ...
    private bool CheckDraggableRectVisibility(RectTransform rectTransform) {
      ...
      // anchor is on top.
      if (anchoredPos.y >= _canvasScaler.referenceResolution.y * .5f + size.y ||
          anchoredPos.y <= -_canvasScaler.referenceResolution.y * .5f)
        return false;
