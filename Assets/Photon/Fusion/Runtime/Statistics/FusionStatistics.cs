...
namespace Fusion.Statistics {
  ...
  public class FusionStatistics : SimulationBehaviour, ISpawned {
    ...
    [InlineHelp] [ExpandableEnum] [SerializeField]
    ...
    private List<FusionStatisticsStatCustomConfig> _statsCustomConfig = new();
    ...
    internal List<FusionStatsGraphBase> ActiveGraphs { get; private set; }
    ...
    private void Awake() {
      ActiveGraphs = new List<FusionStatsGraphBase>();
      _statsCanvasPrefab = Resources.Load<GameObject>(STATS_CANVAS_PREFAB_PATH);
      _objectGraphCombinePrefab =
        Resources.Load<FusionNetworkObjectStatsGraphCombine>(STATS_OBJECT_COMBINE_PREFAB_PATH);

      if (_statsCanvasPrefab == null || _objectGraphCombinePrefab == null) {
        Log.Error(
          $"Error loading the required assets for Fusion Statistics, destroying stats instance. Make sure that the following paths are valid for the Fusion Statistics resource assets: \n 1. {STATS_CANVAS_PREFAB_PATH} \n 2. {STATS_OBJECT_COMBINE_PREFAB_PATH}"
        );
        Destroy(this);
      }
    }
    ...
    private void Update() {
      ...
      // Collect and update
      if (Runner.TryGetFusionStatistics(out var statisticsManager)) UpdateAllGraphs(statisticsManager);
    ...
    /// </summary>
    /// <param name="anchor">
    ///   The FusionStatsWorldAnchor component that defines the anchor object. Null to return to screen
    ///   space overlay.
    /// </param>
    ...
    public bool MonitorNetworkObject(NetworkObject networkObject,
      FusionNetworkObjectStatistics objectStatisticsInstance, bool monitor) {
      if (Runner.TryGetFusionStatistics(out var statisticsManager))
    ...
    private void UpdateAllGraphs(FusionStatisticsManager statisticsManager) {
      ...
      foreach (var statsGraphBase in ActiveGraphs) statsGraphBase.UpdateGraph(Runner, statisticsManager, ref now);
    }
